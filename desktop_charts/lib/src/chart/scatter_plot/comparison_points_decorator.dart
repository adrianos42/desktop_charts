...
class ComparisonPointsDecorator<D> extends PointRendererDecorator<D> {
  const ComparisonPointsDecorator({PointSymbolRenderer? symbolRenderer})
      : symbolRenderer = symbolRenderer ?? const CylinderSymbolRenderer();
  ...
  List<Offset>? computeBoundedPointsForElement(
    ...
    Offset p1 = Offset(point.xLower!, point.yLower!);
    Offset p2 = Offset(point.xUpper!, point.yUpper!);
...
class _Line {
  ...
  factory _Line.fromPoints(Offset p1, Offset p2) {
    ...
    final m = ((p2.dy - p1.dy) / (p2.dx - p1.dx)).toDouble();

    // y-intercept of the line p1p2.
    final b = (p1.dy - (m * p1.dx)).toDouble();
