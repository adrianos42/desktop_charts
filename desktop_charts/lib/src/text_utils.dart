...
List<TextElement> wrapLabelLines(
  ...
  if (maxWidth <= 0.0 || maxLines <= 0) {
    return <TextElement>[];
  }

  TextElement createTextElement(String text) => TextElement(text)
    ..textStyle = textStyle
  ...
  while (labelPartElements.isNotEmpty && currentLineNumber < maxLines) {
    ...
    // If the word can fit in the left space of the line.
    if (width < maxWidth) {
      currentWidth = width;
      ...
      // If the word can not fit in the left space of the line.
